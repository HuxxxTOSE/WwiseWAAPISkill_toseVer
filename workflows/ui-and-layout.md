...
```powershell
python scripts\wwise_waapi.py ak.wwise.ui.commands.getCommands
```
...
python scripts\wwise_waapi.py ak.wwise.ui.commands.execute '{"command": "SaveAll"}'

# Command operating on selected/passed objects
python scripts\wwise_waapi.py ak.wwise.ui.commands.execute '{
...
```powershell
python scripts\wwise_waapi.py ak.wwise.ui.commands.register '{
  "commands": [
    {
...
python scripts\wwise_waapi.py ak.wwise.ui.commands.unregister '{"commands": ["company.tools.normalize_loudness"]}'
```
...
```powershell
python scripts\wwise_waapi.py ak.wwise.ui.getSelectedObjects '{}' '{"return":["id","name","type","path","@Volume"]}'
```

## Project open / close / create

```powershell
python scripts\wwise_waapi.py ak.wwise.ui.project.open '{
  "path": "C:\\Projects\\MyGame\\MyGame.wproj",
  "onMigrationRequired": "migrate",
  "bypassSave": true
}'

python scripts\wwise_waapi.py ak.wwise.ui.project.close '{"bypassSave": true}'

python scripts\wwise_waapi.py ak.wwise.ui.project.create '{
...
```powershell
python scripts\wwise_waapi.py ak.wwise.ui.layout.getLayoutNames
python scripts\wwise_waapi.py ak.wwise.ui.layout.getCurrentLayoutName

# Switch to a built-in layout (e.g. "Designer", "Mixer", "Profiler", "SoundBank")
python scripts\wwise_waapi.py ak.wwise.ui.layout.switchLayout '{"name":"Profiler"}'

# Serialize current/named layout to JSON
python scripts\wwise_waapi.py ak.wwise.ui.layout.getLayout '{"name":"Designer"}'
```
...
```powershell
python scripts\wwise_waapi.py ak.wwise.ui.layout.setLayout '{"name":"MyLayout","layout":{...full Layout dict...}}'
python scripts\wwise_waapi.py ak.wwise.ui.layout.removeLayout '{"name":"MyLayout"}'
```
...
python scripts\wwise_waapi.py ak.wwise.ui.layout.getViewTypes

# What's open right now in a layout?
python scripts\wwise_waapi.py ak.wwise.ui.layout.getViewInstances '{"name":"Designer"}'

# Ensure a view is open and visible
python scripts\wwise_waapi.py ak.wwise.ui.layout.getOrCreateView '{"name":"Property Editor"}'

# Dock a floating view next to another
python scripts\wwise_waapi.py ak.wwise.ui.layout.dockView '{
  "name":"Designer",
  "viewID":"{view-guid}",
...
python scripts\wwise_waapi.py ak.wwise.ui.layout.undockView '{"name":"Designer","viewID":"{view-guid}","posX":100,"posY":100}'

# Move a splitter by relative pixels
python scripts\wwise_waapi.py ak.wwise.ui.layout.moveSplitter '{"id":"{splitter-guid}","delta":-50}'

# Get current rectangle (for a screen capture coordinate)
python scripts\wwise_waapi.py ak.wwise.ui.layout.getElementRectangle '{"id":"{element-guid}"}'
```
...
```powershell
# Whole UI
python scripts\wwise_waapi.py ak.wwise.ui.captureScreen

# Specific view
python scripts\wwise_waapi.py ak.wwise.ui.captureScreen '{"viewName":"Property Editor"}'

# Specific rect (relative to the named view, or to the full window if viewName omitted)
python scripts\wwise_waapi.py ak.wwise.ui.captureScreen '{"viewName":"Property Editor","rect":{"x":0,"y":0,"width":400,"height":300}}'
```
